...
class MemberJoinHandler(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        bg.text((960, 740), f"{member.name} is member no.{member.guild.member_count}!", color="white", font=font_small, align='center')
        
        img_file = discord.File(fp=bg.image_bytes, filename=randomized_image)
        
        await welcome_channel.send(f"Hello there, {member.name}!")
